...
top_10_spam_words <- ham_spam_odds$word[1:10]
...
has_number_y_hat <- ifelse(str_detect(test_set$message, "\\d{5,11}"), "spam", "ham") |>
  factor(levels = levels(test_set$type))
F_meas(has_number_y_hat, reference=test_set$type, relevant="spam", beta=0.75)

# Model 3: Based on words
# TODO: Fix this
has_spam_words_y_hat <- ifelse(any(tokenize_words(test_set$message) %in% top_10_spam_words), "spam", "ham")

# Looking at spambase, try capital letters too
# Classifciation tree
# Random forest
# other models?
# Ensemble?